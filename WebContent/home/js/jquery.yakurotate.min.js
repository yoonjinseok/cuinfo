...
(function ($) {
	$.yakurotate = function (el, options) {
		...
		s.wrapBox = function () {
			s.$container = $("<div class='rotate-container'></div>");
			s.$wrap = $("<div class='rotate-wrap'></div>").css({
				'overflow': 'hidden',
				'position': 'relative',
				'width': s.width,
				'height': s.height
		...
		s.setNav = function () {
			...
			for (var i = 0, cnt = s.itemLength; i < cnt; i++) {
				selectedClass = (i === 0) ? "rotate-num-selected" : "";
				box += "<a href='#' class='" + selectedClass + " rotate-num'>" + (i + 1) + "</a>"
			}
			if (s.opts.navContainer) {
				$(s.opts.navContainer).append(box);
				s.$nav = $(s.opts.navContainer)
			} else {
				s.$nav = $("<div class='rotate-nav'>" + box + "</div>");
				s.$el.parent().parent().append(s.$nav)
			}
			s.$nav.find(".rotate-num").bind("click.rotate", function (event) {
				if($("#rotate-left").is(':animated')) return false; 
				s.stop(); 
		...
		s.setPosition = function () {
			var tot = 0;
			s.$el.children().each(function () {
				tot += (s.opts.move === 'top') ? $(this).height() : $(this).width()
			});
			if (s.opts.move === 'left') {
				s.$el.css({
					'width': tot + 'px',
		...
		s.setelectNav = function (index) {
			if (s.opts.showNav) {
				s.$nav.find(".rotate-num").removeClass("rotate-num-selected").eq((index || s.currentIndex) - 1).addClass("rotate-num-selected")
			}
		};
		s.active = function (index) {
			s.$items.removeClass(s.opts.activeClass).eq(s.currentIndex - 1).addClass(s.opts.activeClass)
		};
		...
		s.setEventBind = function () {
			$(s.opts.nextButton).unbind('click.rotate');
			$(s.opts.prevButton).unbind('click.rotate');
			$(s.opts.playButton).unbind('click.rotate');
			$(s.opts.stopButton).unbind('click.rotate');
			$(s.opts.nextButton).bind('click.rotate', function () {
				if($("#rotate-left").is(':animated')) return false; 
				s.stop(); 
				s.transition('next');
				if (s.opts.autoStart) { 
			...
			$(s.opts.prevButton).bind('click.rotate', function () {
				if($("#rotate-left").is(':animated')) return false; 
				s.stop(); 
				s.transition('prev');
				if (s.opts.autoStart) { 
			...
			$(s.opts.playButton).bind('click.rotate', function () {
				if($("#rotate-left").is(':animated')) return false; 
				s.stop();
				s.play()
			});
			$(s.opts.stopButton).bind('click.rotate', function () {
				if($("#rotate-left").is(':animated')) return false; 
				s.stop()
			})
		};
